\begin{frame}[beamer:1]{Additive Lexical MD Analysis}{\cite{biberconrad2001md,berber2019additive}}
\begin{figure}
\centering
\begin{tikzpicture}
...
\tikzstyle{line} = [-latex,draw=black,line width=.4]
  ...
  \node[block,fill=red!40,text=black] (tag) at (5,3) {POS tag and lemmatize target corpus};
...
%  \node[block,fill=green!40,text=black] (standard) at (7,-1) {Standardize counts};
  ...
  \node[block,fill=orange,text=white] (analyze) at (5,-1.5) {Micro analyses};
  ...
  \draw[line, densely dotted] (selectvars) to[in=90,out=270] (scores);
    ...
    \draw[line] (analyze) to (dimscores);  

%    \draw[line] (analyze) to[in=180,out=180,looseness=2.5] (dimscores);  
